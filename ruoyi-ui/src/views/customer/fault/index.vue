<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['customer:fault:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['customer:fault:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['customer:fault:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="faultList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="考卷id" align="center" prop="examId" />
      <el-table-column label="结束时间" align="center" prop="attempTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.attempTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="考卷id" prop="examId">
          <el-input v-model="form.examId" placeholder="请输入考卷id" />
        </el-form-item>
      </el-form>
...
<script>
import { listFault, getFault, delFault, addFault, updateFault } from "@/api/customer/fault";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户错题表编号为"' + ids + '"的数据项？').then(function() {
        return delFault(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('customer/fault/export', {
        ...this.queryParams
